...
const userFavouriteContent = (droid) => {
  return `
    <li>
      ...
      <div>
        <h4><a class="links" href='/droids/${droid.droid_id}'>${droid.name}</a></h4>
        ...
        <a href='/droids/${droid.droid_id}' class="links btn btn-primary">View <i class="fas fa-robot"></i><a>
      </div>
    </li>
  `;
};
...
const userListings = (droid) => {
  return `
    <li>
      <img src="../${droid.image_url}">
      <div>
      <h4><a class="links" href='/droids/${droid.id}'>${droid.name}</a></h4>
        <small>Date Posted: ${new Date(droid.date_posted).toLocaleDateString()}</small>
        <ul>
          <li>
            <strong class="text-primary list-label">Manufacturer:</strong> ${droid.manufacturer}
